...
- 各書類のソースとなる「統一CSVレイアウト CSV ファイル（以下、統一CSV）」の作成例
...
89  | 介護医療保険料の金額                     | 同上
90  | 新個人年金保険料の金額                   | 同上
138 | 受給者番号                              | 会社で規定する（社内でのみ重複のない）従業員番号でよいが、仕様書を見ると英字や一般的な半角記号は使えないとあるため、本ページでは数字のみで構成される番号を推奨

### 備考

- PCdesk に投入する際、文字コードは UTF-8 でも問題ないことを確認済み
- [給報等統一CSVデータ作成支援ツール 令和07年分](https://www.eltax.lta.go.jp/news/14438)は、手元のエクセルで動かなかった（ファイル読み込みがサイレントに失敗した）ので注意
- [PCdesk のエラーログ](https://www.eltax.lta.go.jp/denshishinkoku/case04/)はバージョン `6.109.2025.1215` では、ログファイル自体が作成されなかったので注意

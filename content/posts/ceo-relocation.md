...
- 電子申請の方法
  ...
  1. 申請書に情報を記入・作成する
      ...
      1. 「○○登記申請書」の○○に `株式会社変更` を入力する
      1. 会社・法人情報を入力する（検索フォームがあるのでそれを利用）
      1. 「登記の事由」に `代表取締役の住所変更` を入力する
      1. 「登記すべき事項」に次のように移転後の住民票の住所を入力する
          ...
          なお、このフォーマットはフォームより転記可能な「作成例」を参考にしたものである
      1. 「登録免許税額」に `10000` を入力する（資本金が 1 億円を超える場合 `30000` を入力）
      1. 「申請年月日」および「申請人」を入力する

  1. 上部メニューにある「署名付与」より、作成した申請書に対し、代表者のマイナンバーカードで電子署名を付与
  1. 「登録免許税 = 10,000 円、または資本金が 1 億円を超える場合 30,000 円」を電子納付（インターネットバンキング）
